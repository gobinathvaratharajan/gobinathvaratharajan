...
const getAssetData = async (authorAvatar?: string) => {
  try {
    ...
    const fetchPromises = [fetch(assetUrls.clashDisplay), fetch(assetUrls.cabinetGrotesk), fetch(assetUrls.logo)];
    ...
    const assetPromises = [clashDisplayRes.arrayBuffer(), cabinetGroteskRes.arrayBuffer(), logoRes.arrayBuffer()];
...
export default async function Image({ params }: { params: { slug: string } }) {
  try {
    const page = await loadBlogPostMeta(params.slug);

    if (!page) {
      return new Response('Blog post not found', { status: 404 });
    }

    const authorKey = page.author as string;
    const authorDetails = authorKey && isValidAuthor(authorKey) ? getAuthor(authorKey as AuthorKey) : null;
        ...
        >
          <div style={styles.container}>
            <div style={styles.titleContainer}>
              ...
              {page.description && <p style={styles.summary}>{page.description}</p>}
            </div>
            <div style={styles.metaContainer}>
              {authorDetails && (
                <div style={{ ...styles.metaBase, ...styles.authorMeta }}>
                  {(assetData?.authorAvatarBase64 || authorDetails.avatar) && (
                    <img
                      src={
                        assetData?.authorAvatarBase64 || `${process.env.NEXT_PUBLIC_SITE_URL}${authorDetails.avatar}`
                      }
              ...
              {authorDetails && page.date && <span style={styles.dotSeparator}>•</span>}
              {page.date && <p style={{ ...styles.metaBase, ...styles.dateMeta }}>{formatDate(page.date)}</p>}
            </div>
          </div>
        </div>
      ),
